...
function buildManualPanel() {
  const panelLines = [
    'Escolha um guia e sinta o coração do cantinho bater baixinho. 🧸💗',
    'Abra um manual e entre no modo aconchego do Nózinho. 🌸🤍',
    'Pegue um chá, respire fundo e escolha por onde explorar. ✨'
  ];

  const embed = new EmbedBuilder().setColor(0xf9a8d4).setTitle('Manuais do Nózinho ✧˖°').setDescription(panelLines[Math.floor(Math.random() * panelLines.length)]);
...
async function handleManualButtons(interaction) {
  if (interaction.customId === 'manual:nerissa') {
    ...
    const embed = new EmbedBuilder().setColor(0xf472b6).setTitle('Manual da Nerissa ୨ৎ').setDescription('Use /nozinho para abrir o painel principal.\nCrie cartinhas, músicas e cantinhos com pistas e carinho.\nUse /segredo quando quiser tocar uma área mais íntima.\nAlgumas partes só florescem com senha e desbloqueios. 💌✨').setImage('https://i.pinimg.com/originals/4a/4a/86/4a4a86d520abe35d1204f7768869e63b.gif');
  ...
  if (interaction.customId === 'manual:ramiel') {
    ...
    const embed = new EmbedBuilder().setColor(0xa78bfa).setTitle('Manual do Ramiel ✧').setDescription('O Nózinho é um cofre emocional feito com cuidado.\nExplore pistas, tente palavras e desbloqueie partes aos poucos.\nCartas, músicas e lembranças podem surgir no tempo certo. 🌙🤍').setImage('https://i.pinimg.com/originals/ff/92/46/ff9246b40bfa0b00721cc1b8b887fad2.gif');
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
